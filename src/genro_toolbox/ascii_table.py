...
def compute_col_widths(names,rows,max_width=120,minw=6,pad=1):
    ...
    for i,n in enumerate(names):
        ...
        for r in rows: ideal=max(ideal,len(strip_ansi(str(r[i]))))
...
def wrap_row(row,widths):
    ...
    for cell,width in zip(row,widths):
        ...
        if has_long_word:
            ...
            result.append(textwrap.wrap(s,width,break_long_words=True,break_on_hyphens=False) or [""])
        else:
            # Can wrap without breaking words
            result.append(textwrap.wrap(s,width,break_long_words=False,break_on_hyphens=False) or [""])
    return result
